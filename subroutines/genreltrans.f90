...
!         ionvar: sets the ionisation variation (1 = w/ ion var; 0 = w/o ion var)
   ...
   ! complex, dimension(:,:,:,:,:), allocatable :: transe, transea
    ...
    if( config%flo .lt. tiny(config%flo) .or. config%fhi .lt. tiny(config%fhi) )then
        ...
        model_args%beta_p = 1. !this is an ugly hack for the double LP model, to calculate the time-averaged spectrum
    else
        DC     = 0
    ...
    call need_check(Cp,Cpsave,param,paramsave,config%fhi,config%flo,fhisave,flosave,config%nf,prev_nf,needtrans,needconv)
    ...
    if( needtrans )then
       ...
       status_re_tau = .true.       
       call rtrans(config%verbose,dset,nlp,model_args%a,model_args%h,muobs,model_args%Gamma,model_args%rin,model_args%rout,model_args%honr,d,rnmax,model_args%zcos,model_args%b1,model_args%b2,model_args%qboost,model_args%eta_0,&
                    fcons,config%nro,config%nphi,nex,config%dloge,config%nf,config%fhi,config%flo,config%me,config%xe,arrays%ker_W0,arrays%ker_W1,arrays%ker_W2,arrays%ker_W3,frobs,frrel)
       ! print *, 'gso ', gso(1)
    end if
    ...
    if( dset .eq. 0 .or. size(model_args%h) .eq. 2) then
       ...
       call init_cont(nlp,model_args%a,model_args%h,model_args%zcos,model_args%Cutoff_s,model_args%Cutoff_obs,model_args%logxi, model_args%lognep, muobs,Cp_cont,Cp,fcons,model_args%Gamma,&
                   model_args%Dkpc,model_args%Mass,arrays%earx,config%Emin,config%Emax,contx,config%dloge,config%verbose,dset,model_args%Anorm,contx_int,model_args%eta)

       call radfunctions_dens(config%verbose,config%xe,model_args%rin,rnmax,model_args%eta_0,dble(model_args%logxi),dble(model_args%lognep),model_args%a,model_args%h,model_args%Gamma,model_args%honr,&
            rlp,dcosdr,cosd,contx_int,ndelta,nlp,config%rmin,npts,logxir,gsdr,logner,dfer_arr)
    else
        call radfuncs_dist(config%xe,model_args%rin,rnmax,model_args%b1,model_args%b2,model_args%qboost,fcons,&
                           & dble(model_args%lognep),model_args%a,model_args%h(1),model_args%honr,rlp,dcosdr,cosd,ndelta,config%rmin,npts(1),&
    ...
    if( needconv )then
        ...
        call xilimits(nex,arrays%earx,nlp,contx,DeltaGamma,real(gso),real(lens),real(model_args%zcos),dlogxi1,dlogxi2)
        ...
        do rbin = 1, config%xe  !Loop over radial zones
            ...
            do mubin = 1, config%me      !loop over emission angle zones
                ...
                call rest_frame(arrays%earx,nex,Gamma0,model_args%Afe,logne,Cutoff_0,logxi0,thetae,Cp,photarx)
                !NON LINEAR EFFECTS
                if (DC .eq. 0) then 
                   !Gamma variations
                   logxi1 = logxi0 + ionvariation * dlogxi1
                   call rest_frame(arrays%earx,nex,Gamma1,model_args%Afe,logne,Cutoff_0,logxi1,thetae,Cp,photarx_1)
                   logxi2 = logxi0 + ionvariation * dlogxi2
                   call rest_frame(arrays%earx,nex,Gamma2,model_args%Afe,logne,Cutoff_0,logxi2,thetae,Cp,photarx_2)
                   photarx_delta = (photarx_2 - photarx_1)/(Gamma2-Gamma1)
                   !xi variations
                   call rest_frame(arrays%earx,nex,Gamma0,model_args%Afe,logne,Cutoff_0,logxi1,thetae,Cp,photarx_1)
                   call rest_frame(arrays%earx,nex,Gamma0,model_args%Afe,logne,Cutoff_0,logxi2,thetae,Cp,photarx_2)
                ...
                do j = 1,config%nf
                   ...
                   if (test) then
                      call conv_one_FFT(dyn,arrays%earx,Gamma0,Hx,reline_w0,imline_w0,arrays%ReW0(:,:,j),arrays%ImW0(:,:,j),DC,nlp)
                      if(DC .eq. 0 .and. config%refvar .eq. 1) then
                         call conv_one_FFT(dyn,arrays%earx,Gamma0,Hx,reline_w1,imline_w1,arrays%ReW1(:,:,j),arrays%ImW1(:,:,j),DC,nlp)
                         call conv_one_FFT(dyn,arrays%earx,Gamma0,Hx_delta,reline_w2,imline_w2,arrays%ReW2(:,:,j),arrays%ImW2(:,:,j),DC,nlp)
                      end if
                      if(DC .eq. 0 .and. config%ionvar .eq. 1) then
                         call conv_one_FFT(dyn,arrays%earx,Gamma0,Hx_dlogxi,reline_w3,imline_w3,arrays%ReW3(:,:,j),arrays%ImW3(:,:,j),DC,nlp)
                      end if
                   else
                      call conv_one_FFTw(dyn,arrays%earx,Gamma0,Hx,reline_w0,imline_w0,arrays%ReW0(:,:,j),arrays%ImW0(:,:,j),DC,nlp)
                      if(DC .eq. 0 .and. config%refvar .eq. 1) then
                         call conv_one_FFTw(dyn,arrays%earx,Gamma0,Hx,reline_w1,imline_w1,arrays%ReW1(:,:,j),arrays%ImW1(:,:,j),DC,nlp)
                         call conv_one_FFTw(dyn,arrays%earx,Gamma0,Hx_delta,reline_w2,imline_w2,arrays%ReW2(:,:,j),arrays%ImW2(:,:,j),DC,nlp)
                      end if
                      if(DC .eq. 0 .and. config%ionvar .eq. 1) then
                         call conv_one_FFTw(dyn,arrays%earx,Gamma0,Hx_dlogxi,reline_w3,imline_w3,arrays%ReW3(:,:,j),arrays%ImW3(:,:,j),DC,nlp)
                      end if
                   endif
                end do
            end do
    ...
    if( model_args%ReIm .eq. 7 ) then
        ...
        if(nlp .gt. 1 .and. model_args%beta_p .eq. 0. ) then
            call lag_freq_nocoh(nex,arrays%earx,config%nf,arrays%fix,real(config%flo),real(config%fhi),config%Emin,config%Emax,nlp,contx,absorbx,real(tauso),real(gso),&
                                arrays%ReW0,arrays%ImW0,arrays%ReW1,arrays%ImW1,arrays%ReW2,arrays%ImW2,arrays%ReW3,arrays%ImW3,real(model_args%h),real(model_args%zcos),real(model_args%Gamma),real(model_args%eta),model_args%boost,&
                                model_args%g,model_args%DelAB,config%ionvar,arrays%ReGbar,arrays%ImGbar)
        else
            call lag_freq(nex,arrays%earx,config%nf,arrays%fix,real(config%flo),real(config%fhi),config%Emin,config%Emax,nlp,contx,absorbx,real(tauso),real(gso),&
                          arrays%ReW0,arrays%ImW0,arrays%ReW1,arrays%ImW1,arrays%ReW2,arrays%ImW2,arrays%ReW3,arrays%ImW3,real(model_args%h),real(model_args%zcos),real(model_args%Gamma),real(model_args%eta),model_args%beta_p,&
                          model_args%boost,model_args%g,model_args%DelAB,config%ionvar,arrays%ReGbar,arrays%ImGbar)        
        end if
    else if (nlp .gt. 1 .and. model_args%beta_p .eq. 0.) then
        call rawG(nex,arrays%earx,config%nf,real(config%flo),real(config%fhi),nlp,contx,absorbx,real(tauso),real(gso),arrays%ReW0,arrays%ImW0,&
                  arrays%ReW1,arrays%ImW1,arrays%ReW2,arrays%ImW2,arrays%ReW3,arrays%ImW3,real(model_args%h),real(model_args%zcos),real(model_args%Gamma),real(model_args%eta),model_args%boost,model_args%ReIm,model_args%g,model_args%DelAB,&
                  config%ionvar,DC,model_args%resp_matr,arrays%ReGrawa,arrays%ImGrawa)
    else
        !Calculate raw FT of the full spectrum without absorption
        call rawS(nex,arrays%earx,config%nf,real(config%flo),real(config%fhi),nlp,contx,real(tauso),real(gso),arrays%ReW0,arrays%ImW0,arrays%ReW1,arrays%ImW1,arrays%ReW2,arrays%ImW2,arrays%ReW3,arrays%ImW3,&
             real(model_args%h),real(model_args%zcos),real(model_args%Gamma),real(model_args%eta),model_args%beta_p,model_args%boost,model_args%g,model_args%DelAB,config%ionvar,DC,arrays%ReSraw,arrays%ImSraw)
    ...
    if( DC .eq. 1 )then
        ...
        if(nlp .eq. 1 .or. model_args%beta_p .ne. 0.) then
            if (model_args%ReIm .gt. 0.0) then
                call propercross(nex, config%nf, arrays%earx, arrays%ReSrawa, arrays%ImSrawa, arrays%ReGrawa, arrays%ImGrawa, model_args%resp_matr)
            else
                call propercross_NOmatrix(nex, config%nf, arrays%earx, arrays%ReSrawa, arrays%ImSrawa, arrays%ReGrawa, arrays%ImGrawa)
            endif
        end if
    ...
    if( model_args%ReIm .eq. 7 ) then
        ...
        call crebin(nex,arrays%earx,arrays%ReGbar,arrays%ImGbar,ne,ear,ReS,ImS) !S is in photar form
        ! do i = 1, ne
        ...
        call cfoldandbin(nex,arrays%earx,arrays%ReGbar,arrays%ImGbar,ne,ear,ReS,ImS,model_args%resp_matr) !S is count rate
    ...
    if (config%verbose .gt. 1 .and. abs(model_args%ReIm) .gt. 0 .and. model_args%ReIm .lt. 7) then
        if( DC .eq. 0 .and. model_args%beta_p .eq. 0) then
           call write_components(ne,ear,nex,arrays%earx,config%nf,real(config%flo),real(config%fhi),nlp,contx,absorbx,real(tauso),real(gso),&
                                  arrays%ReW0,arrays%ImW0,arrays%ReW1,arrays%ImW1,arrays%ReW2,arrays%ImW2,arrays%ReW3,arrays%ImW3,real(model_args%h),real(model_args%zcos),real(model_args%Gamma),real(model_args%eta),&
                                  model_args%beta_p,model_args%boost,model_args%floHz,model_args%fhiHz,model_args%ReIm,model_args%DelA,model_args%DelAB,model_args%g,config%ionvar,model_args%resp_matr)
